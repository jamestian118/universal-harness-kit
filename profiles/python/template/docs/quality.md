...
- 原则 5：artifacts 生命周期必须走 `finalize-artifacts` 闸门（plan -> confirm -> apply），且仅清理 git ignored 的 `.artifacts/`、`test-results/` 范围
...
- Principle 5: Artifact lifecycle must go through the `finalize-artifacts` gate (plan -> confirm -> apply), and cleanup is limited to git-ignored `.artifacts/` and `test-results/`.
