...
### prereqs
- 选择并安装你的语言工具链（例如 python/node/go/rust 等）
...
```
- 方案 2 + 合规闸门：先生成 plan，再确认，再执行删除
- `apply` 默认先运行 `./scripts/verify`，只会删除 plan 中且满足“位于 roots 下 + git ignored”的候选
- 默认 roots：`.artifacts`、`test-results`、`scripts/.tmp`、`.ai/tmp-scripts`

### milestone-finalize
```bash
./scripts/milestone-finalize
```
- milestone 封板入口：自动执行 `./scripts/verify` 与 `finalize-artifacts dry-run`
- 若 `CI=true` 或非 TTY，输出“跳过交互清理”，不会删除
- 交互式 TTY 下输入 `yes` 时自动执行 confirm + apply
...
### warning
- generic 模板中的 `./scripts/setup`、`./scripts/dev`、`./scripts/format`、`./scripts/lint`、`./scripts/test` 默认是占位实现
- 在这些脚本替换为项目真实实现前，`./scripts/verify` 预期会失败（属正常现象）
- 先完成脚本替换，再将 `verify` 作为提交/CI gate
...
./scripts/arch-check
```
- 检查 src/ 中是否存在违规 import tests/ 的情况
- 违规：输出文件名和行号，exit 1
- 无违规：输出 OK，exit 0
...
### prereqs
- install your language toolchain (e.g., python/node/go/rust, etc.)
...
./scripts/finalize-artifacts apply --token "<token>"
```
- Scheme 2 + compliance gate: generate plan, confirm it, then apply deletion
- `apply` runs `./scripts/verify` by default and only deletes planned candidates that are both under roots and git ignored
- Default roots: `.artifacts`, `test-results`, `scripts/.tmp`, `.ai/tmp-scripts`

### milestone-finalize
```bash
./scripts/milestone-finalize
```
- Milestone close-out entrypoint: automatically runs `./scripts/verify` and `finalize-artifacts dry-run`
- When `CI=true` or no TTY is available, it prints "skip interactive cleanup" and does not delete
- In interactive TTY mode, entering `yes` runs confirm + apply automatically
...
### warning
- In the generic template, `./scripts/setup`, `./scripts/dev`, `./scripts/format`, `./scripts/lint`, and `./scripts/test` are placeholders by default
- Until those scripts are replaced with real project implementations, `./scripts/verify` is expected to fail (this is normal)
- Replace the scripts first, then treat `verify` as a commit/CI gate
...
./scripts/arch-check
```
- Checks whether src/ contains any forbidden imports from tests/
