...
- tests 分级：smoke（必须）→ integration（推荐）→ edge/adversarial（关键路径推荐）；新增关键功能需附 eval 说明
...
- Test tiers: smoke (required) → integration (recommended) → edge/adversarial (recommended for critical paths); new critical features should include eval notes
