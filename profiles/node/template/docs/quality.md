...
- 原则 5：artifacts 清理必须通过 `finalize-artifacts` 合规闸门（plan + confirm + apply）；禁止直接 `rm -rf` 绕过审计
...
- Principle 5: Artifact cleanup must go through the `finalize-artifacts` gate (plan + confirm + apply); do not bypass audit with direct `rm -rf`
